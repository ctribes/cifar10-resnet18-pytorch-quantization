...
from .. import Bottleneck, BasicBlock, ResNet
...
#           'resnext101_32x8d']
#
#model_local_checkpoints = {
#'resnet18_fp' : 'checkpoint/resnet18-5c106cde.pth',
#'resnet18_quant' : 'checkpoint/ckpt_qt.t7'
#}
...
def _resnet(arch, block, layers, pretrained, pretrained_checkpoint, num_classes, quantize, **kwargs):
    model = QuantizableResNet(block, layers, num_classes, **kwargs)
    _replace_relu(model)
    ...
    if quantize:
        ...
        if pretrained:
            print('==> Loading pretrained full precision for quantization from ',pretrained_checkpoint)
        ...
        if pretrained:
            print('==> Loading pretrained full precision from ',pretrained_checkpoint)
            checkpoint = torch.load(pretrained_checkpoint)
            model.load_state_dict(checkpoint)
...
def resnet18(pretrained=False, pretrained_checkpoint='./checkpoint/resnet18_cifar10.pth', num_classes=10, quantize=False, **kwargs):
